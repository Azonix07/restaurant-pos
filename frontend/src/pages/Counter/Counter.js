...
import { FiClock, FiDollarSign, FiCheck, FiAlertTriangle } from 'react-icons/fi';
...
const Counter = () => {
  ...
  const handleOpen = async (e) => {
    e.preventDefault();
    try {
      await api.post('/counter/open', { openingCash: parseFloat(openingCash) || 0 });
  ...
  const handleClose = async (e) => {
    ...
    try {
      await api.post('/counter/close', {
        declaredCash: parseFloat(closeForm.declaredCash) || 0,
          ...
          {session ? (
            ...
            <div className="card counter-closed-msg">
              ...
              <form onSubmit={handleOpen} style={{ maxWidth: 400, margin: '0 auto' }}>
                <div className="input-group"><label>Opening Cash (₹)</label><input className="input" type="number" min="0" step="0.01" value={openingCash} onChange={e => setOpeningCash(e.target.value)} required placeholder="Count the cash in drawer" /></div>
